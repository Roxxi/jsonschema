...
(deftest mysql-mediumblob->json-type-test []
  (let [str-type (dt/col-type->json-type mysql-tt "mediumblob")]
    (is (= (type str-type) jsonschema.type_system.types.Str))
    (is (= (types/getMin str-type) 0))
    (is (= (types/getMax str-type) (- (math/expt 2 24) 1)))))

(deftest mysql-longblob->json-type-test []
  (let [str-type (dt/col-type->json-type mysql-tt "longblob")]
    (is (= (type str-type) jsonschema.type_system.types.Str))
    (is (= (types/getMin str-type) 0))
    (is (= (types/getMax str-type) (- (math/expt 2 32) 1)))))
...
(deftest mysql-mediumtext->json-type-test []
  (let [str-type (dt/col-type->json-type mysql-tt "mediumtext")]
    (is (= (type str-type) jsonschema.type_system.types.Str))
    (is (= (types/getMin str-type) 0))
    (is (= (types/getMax str-type) (- (math/expt 2 24) 1)))))

(deftest mysql-longtext->json-type-test []
  (let [str-type (dt/col-type->json-type mysql-tt "longtext")]
    (is (= (type str-type) jsonschema.type_system.types.Str))
    (is (= (types/getMin str-type) 0))
    (is (= (types/getMax str-type) (- (math/expt 2 32) 1)))))
...
(deftest schema-int->mysql-str []
  (let [schema-str (types/make-str 65536 65536)
        mysql-str (dt/json-type->col-type mysql-tt schema-str)]
    (is (= mysql-str "mediumtext"))))

(def MEDIUMTEXT_LENGTH_MAX (- (math/expt 2 24) 1))

(deftest schema-int->mysql-str []
  (let [schema-str (types/make-str MEDIUMTEXT_LENGTH_MAX MEDIUMTEXT_LENGTH_MAX)
        mysql-str (dt/json-type->col-type mysql-tt schema-str)]
    (is (= mysql-str "mediumtext"))))
...
(deftest schema-int->mysql-str []
  (let [schema-str (types/make-str LONGTEXT_LENGTH_MAX LONGTEXT_LENGTH_MAX)
        mysql-str (dt/json-type->col-type mysql-tt schema-str)]
    (is (= mysql-str "longtext"))))
